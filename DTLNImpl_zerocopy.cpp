...
void DTLN_RKNN(){
    ...
    for (i=0;i<m_ioNum[0].n_output;i++){
     ...
     for (i=0;i<m_ioNum[0].n_input;i++){
        input_attrs_a[i].fmt=RKNN_TENSOR_NHWC;
        input_attrs_b[i].fmt=RKNN_TENSOR_NHWC;
        input_attrs_a[i].type=RKNN_TENSOR_FLOAT32;
        input_attrs_b[i].type=RKNN_TENSOR_FLOAT32;

        if(i<1){
            
            input_attrs_a[i].size=FFT_OUT_SIZE*sizeof(float);
            input_attrs_a[i].size_with_stride=FFT_OUT_SIZE*sizeof(float);
            input_attrs_b[i].size=DTLN_BLOCK_LEN*sizeof(float);
            input_attrs_b[i].size_with_stride=DTLN_BLOCK_LEN*sizeof(float);
            m_pEngine->m_input_details_a[i]= rknn_create_mem(m_ctx[0],FFT_OUT_SIZE*sizeof(float));
            m_pEngine->m_input_details_b[i]= rknn_create_mem(m_ctx[1],DTLN_BLOCK_LEN*sizeof(float));
        }else{
            input_attrs_a[i].size=DTLN_UNIT_SIZE*sizeof(float);
      
            input_attrs_a[i].size_with_stride=DTLN_UNIT_SIZE*sizeof(float);
            input_attrs_b[i].size=DTLN_UNIT_SIZE*sizeof(float);
            input_attrs_b[i].size_with_stride=DTLN_UNIT_SIZE*sizeof(float);

            m_pEngine->m_input_details_a[i]= rknn_create_mem(m_ctx[0],DTLN_UNIT_SIZE*sizeof(float));
            m_pEngine->m_input_details_b[i]= rknn_create_mem(m_ctx[1],DTLN_UNIT_SIZE*sizeof(float));
        }
        rknn_set_io_mem(m_ctx[0],m_pEngine->m_input_details_a[i],&input_attrs_a[i]);
        rknn_set_io_mem(m_ctx[1],m_pEngine->m_input_details_b[i],&input_attrs_b[i]);

    }

    for (i=0;i<m_ioNum[0].n_output;i++){
        output_attrs_a[i].type=RKNN_TENSOR_FLOAT32;
        output_attrs_b[i].type=RKNN_TENSOR_FLOAT32;
       
        if(i<1){
           
            output_attrs_a[i].size=FFT_OUT_SIZE*sizeof(float);
            output_attrs_b[i].size=DTLN_BLOCK_LEN*sizeof(float);
            
            m_pEngine->m_output_details_a[i]= rknn_create_mem(m_ctx[0],FFT_OUT_SIZE*sizeof(float));
            m_pEngine->m_output_details_b[i]= rknn_create_mem(m_ctx[1],DTLN_BLOCK_LEN*sizeof(float));

        }else{
            
            output_attrs_a[i].size=DTLN_UNIT_SIZE*sizeof(float);
            output_attrs_b[i].size=DTLN_UNIT_SIZE*sizeof(float);
            
            m_pEngine->m_output_details_a[i]= rknn_create_mem(m_ctx[0],DTLN_UNIT_SIZE*sizeof(float));
            m_pEngine->m_output_details_b[i]= rknn_create_mem(m_ctx[1],DTLN_UNIT_SIZE*sizeof(float));
           
        }
        rknn_set_io_mem(m_ctx[0],m_pEngine->m_output_details_a[i],&(output_attrs_a[i]));
        rknn_set_io_mem(m_ctx[1],m_pEngine->m_output_details_b[i],&(output_attrs_b[i]));
    ...
    for (int i=0;i<MODEL_INPUT_NUM;i++){
        rknn_destroy_mem(m_ctx[0],m_pEngine->m_input_details_a[i]);
        rknn_destroy_mem(m_ctx[1],m_pEngine->m_input_details_b[i]);
        rknn_destroy_mem(m_ctx[0],m_pEngine->m_output_details_a[i]);
        rknn_destroy_mem(m_ctx[1],m_pEngine->m_output_details_b[i]);
    }
    for (i=0;i<2:i++){
        if(m_ctx[i]>0){
...
void RKNNInfer(trg_engine* m_pEngine, rknn_context* m_ctx) {
    ...
    memcpy(m_pEngine->m_input_details_a[0]->virt_addr,in_mag,FFT_OUT_SIZE*sizeof(float));
    for (int i=0;i<m_pEngine->statesa.size();i++){
        memcpy(m_pEngine->m_input_details_a[i+1]->virt_addr,m_pEngine->statesa[i].data(),DTLN_UNIT_SIZE*sizeof(float));

    }
   
    res2=rknn_run(m_ctx[0], nullptr);

    out_mask =(float*)(m_pEngine->m_output_details_a[0]->virt_addr);
    //output states will become input states of next frame
    for (int i=0;i<m_pEngine->statesa.size();i++){
        memcpy(m_pEngine->statesa[i].data(),(float*)(m_pEngine->m_output_details_a[i+1]->virt_addr),DTLN_UNIT_SIZE*sizeof(float));
    }
    ...
    memcpy(m_pEngine->m_input_details_b[0]->virt_addr,estimated_block,DTLN_BLOCK_LEN*sizeof(float));
    for (int i=0;i<4;i++){
        memcpy(m_pEngine->m_input_details_b[i+1]->virt_addr,m_pEngine->statesb[i].data(),DTLN_UNIT_SIZE*sizeof(float));
    ...
    out_block =(float*)(m_pEngine->m_output_details_b[0]->virt_addr);
    
    for (int i=0;i<4;i++){
        memcpy(m_pEngine->statesb[i].data(),(float*)(m_pEngine->m_output_details_b[i+1]->virt_addr),DTLN_UNIT_SIZE*sizeof(float));
    }
